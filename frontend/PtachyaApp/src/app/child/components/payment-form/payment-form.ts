import { ChangeDetectionStrategy, Component, ElementRef, OnInit, ViewChild, AfterViewInit, signal } from '@angular/core';
...
@Component({
  ...
  templateUrl: './payment-form.html', // ⭐️ מפנה לקובץ HTML נפרד
  styleUrls: ['./payment-form.css'],  // ⭐️ מפנה לקובץ CSS נפרד
  standalone: true,
...
export class PaymentForm implements OnInit, AfterViewInit {
  ...
  onSubmit(): void {
    ...
    const formData = {
        ...
        // כאן נשלחים נתונים אלו לשרת ה-C#
    };

    console.log('טופס תקין. נתוני הטופס המוכנים לשליחה:', formData);

    this.submissionMessage.set('✅ הטופס נשלח בהצלחה לשרת! הנתונים מוכנים להפקת מסמך הוורד/PDF.');
  }
