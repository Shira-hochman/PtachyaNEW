...
            // שמירת שם הקובץ שנבחר
            const fileName = input.files[0].name;
            control.setValue(fileName);
            this.submissionMessage.set(`✅ קובץ "${fileName}" נבחר בהצלחה.`);
        } else {
            // אם לא נבחר קובץ
            control.setValue('');
...
    }
    
    // סינון שדות ריקים במערך childrenInCustody
...
    console.log('טופס תקין. נתוני הטופס המוכנים לשליחה:', formData);

    // ⭐️⭐️⭐️ קריאה לשירות החדש והפעלת ההורדה ⭐️⭐️⭐️
    this.formService.submitDiscountRequest(formData).subscribe({
      next: (response: Blob) => {
              // לוגיקת הורדה: אם קיבלנו PDF, הורד אותו
              const url = window.URL.createObjectURL(response);
              const a = document.createElement('a');
              a.href = url;
              a.download = `Discount_Request_${formData.studentDetails.studentId}.pdf`; 
              
...
          },
          error: (err) => {
              // טיפול בשגיאת HTTP מהשרת (כמו 401, 403, 500)
              console.error('שגיאה בשליחת הטופס מהשרת:', err);
              let msg = '🔴 שגיאה בשליחת הטופס. אנא נסה שנית.';
              if (err.status === 401 || err.status === 403) {
                 msg = '🔴 שגיאת הרשאה! אנא התחבר מחדש ונסה שוב (טוקן פג תוקף).';
              } else if (err.error?.message) {
                 msg = `🔴 שגיאה: ${err.error.message}`; // קורא שגיאות פייתון או שגיאות קוד
              }
              this.submissionMessage.set(msg);
          }
    });
  }
  // איפוס הטופס
